...
| `environment_fixtures.py` | No | `temp_shows_root`, `isolated_cache_manager`, `caching_enabled`, `temp_cache_dir`, `cache_manager`, `shot_cache`, `scene_disk_cache`, `make_test_shot`, `make_test_filesystem`, `make_real_3de_file`, `real_shot_model` | Temporary paths, isolated cache instances, cache-enabled test environments, shot/filesystem factories |
