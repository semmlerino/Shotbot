...
# Cache TTL used when calling read_json_cache directly
_CACHE_TTL = timedelta(minutes=DEFAULT_TTL_MINUTES)
...
class TestMalformedJsonRecovery:
    ...
    @pytest.fixture
    def cache_manager(self, tmp_path: Path) -> ShotDataCache:
        """Create ShotDataCache with isolated cache directory."""
        cache_path = tmp_path / "cache"
        cache_path.mkdir(parents=True, exist_ok=True)
        return ShotDataCache(cache_path)

    def test_recover_from_malformed_content(self, cache_manager: ShotDataCache) -> None:
        ...
        for case_name, payload in malformed_cases:
            ...
            result = read_json_cache(cache_file, _CACHE_TTL, check_ttl=False)
            assert result is None, f"Expected None for malformed case: {case_name}"

    def test_recover_from_wrong_encoding(self, cache_manager: ShotDataCache) -> None:
...
class TestInvalidDataStructureRecovery:
    ...
    def test_accepts_valid_data_structures(self, cache_manager: ShotDataCache) -> None:
...
class TestPermissionErrorRecovery:
    ...
    def test_recover_from_unreadable_file(
        self, cache_manager: ShotDataCache, tmp_path: Path
    ) -> None:
    ...
    def test_recover_from_unwritable_directory(
        ...
        try:
            ...
            success = write_json_cache(cache_file, shots)
...
class TestAtomicWriteGuarantees:
    ...
    def test_successful_write_is_readable(
        ...
        write_json_cache(cache_file, shots)
        result = read_json_cache(cache_file, _CACHE_TTL, check_ttl=False)
    ...
    def test_write_overwrites_existing(self, cache_manager: ShotDataCache) -> None:
        """Write overwrites existing cache file."""
        cache_file = cache_manager.shots_cache_file

        # Write first version
        shots_v1 = [make_shot_dict(shot="0010")]
        write_json_cache(cache_file, shots_v1)

        # Write second version
        shots_v2 = [make_shot_dict(shot="0020"), make_shot_dict(shot="0030")]
        write_json_cache(cache_file, shots_v2)
...
class TestWriteFailureRecovery:
    ...
    def test_invalid_data_type_fails_gracefully(
        ...
        success = write_json_cache(cache_file, invalid_data)
    ...
    def test_write_failure_preserves_existing_cache(
        ...
        invalid_data = [{"obj": NotSerializable()}]
        write_json_cache(cache_file, invalid_data)
...
class TestPublicAPIRecovery:
    ...
    def test_clear_cache_removes_files(self, cache_manager: ShotDataCache) -> None:
        ...
        def read_cache() -> None:
            try:
                result = read_json_cache(
                    cache_manager.shots_cache_file, _CACHE_TTL, check_ttl=False
                )
                with lock:
