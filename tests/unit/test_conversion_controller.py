...
class TestConversionWorkflow:
    ...
    def test_start_conversion_success(self):
        ...
        self.mock_process_manager.start_batch.assert_called_once_with(file_paths, True, 4)
    ...
    def test_start_conversion_already_converting(self):
        ...
        result = self.controller.start_conversion(
            ...
            codec_idx=0, hwdecode_idx=0, crf_value=18,
            parallel_enabled=False, max_parallel=1,
            delete_source=False, overwrite_mode=True
        )
...
class TestAutoBalanceLogic:
    ...
    def test_auto_balance_with_rtx40(self, mock_rtx40_detection):
        ...
        gpu_files = sum(1 for codec in self.controller.file_codec_assignments.values() if codec in [0, 1, 2])
        cpu_files = sum(1 for codec in self.controller.file_codec_assignments.values() if codec == 3)
    ...
    def test_auto_balance_without_rtx40(self, mock_rtx40_detection):
        ...
        self.controller._auto_balance_workload(file_paths, 2)  # AV1 NVENC (requires RTX40)
    ...
    def test_auto_balance_disabled(self):
        ...
        # Should not create any codec assignments
        assert len(self.controller.file_codec_assignments) == 0
...
class TestFFmpegArgumentBuilding:
    ...
    def test_build_ffmpeg_args_h264_nvenc(self, mock_encoder_config, mock_audio_config, mock_hw_config):
        ...
        mock_encoder_config.return_value = (["-c:v", "h264_nvenc", "-preset", "fast"], "Using H.264 NVENC")
        
        input_path = "/test/input.ts"
        output_path = "/test/output.mp4"
        
        with patch.object(self.controller, '_get_output_path', return_value=output_path):
    ...
    @patch('codec_helpers.CodecHelpers.get_audio_codec_args')
    @patch('codec_helpers.CodecHelpers.get_encoder_configuration')
    def test_build_ffmpeg_args_x264_cpu(self, mock_encoder_config, mock_audio_config, mock_hw_config):
        """Test FFmpeg args for x264 CPU encoding"""
        # Configure mocks
        mock_hw_config.return_value = ([], "No hardware acceleration")
        mock_audio_config.return_value = (["-c:a", "aac", "-b:a", "192k"], "Converting to AAC")
        mock_encoder_config.return_value = (["-c:v", "libx264", "-crf", "18"], "Using x264")
    ...
    def test_get_output_path_generation(self):
        ...
        with patch('codec_helpers.CodecHelpers.get_output_extension', return_value=".mp4"):
            output_path = self.controller._get_output_path(input_path, 0)
        
        # Should generate path with _RC suffix
    ...
    def test_get_output_path_different_extensions(self):
        ...
        for input_path, extension, expected in test_cases:
            with patch('codec_helpers.CodecHelpers.get_output_extension', return_value=extension):
                output_path = self.controller._get_output_path(input_path, 0)
                assert output_path == expected
...
class TestProcessManagement:
    ...
    def test_process_next_with_queue(self):
        ...
        with patch.object(self.controller, '_build_ffmpeg_args', return_value=["-i", "input.ts", "output.mp4"]) as mock_build_args:
            with patch.object(self.controller, '_get_codec_for_path', return_value=0) as mock_get_codec:
                self.controller._process_next()
        
        # Should start processing first file
        assert len(self.controller.queue) == 1  # One file removed from queue
...
class TestThreadOptimization:
    ...
    def test_optimize_threads_for_codec(self, mock_optimize):
        ...
        assert threads == 6
        mock_optimize.assert_called_once_with(3, True, self.controller.file_codec_assignments)
...
class TestConversionFinalization:
    ...
    def test_on_process_finished_continue_processing(self):
        ...
        with patch.object(self.controller, '_process_next') as mock_process_next:
            self.controller._on_process_finished(mock_process, 0, "")  # Success
    ...
    def test_on_process_finished_conversion_complete(self):
        ...
        with patch.object(self.controller, '_finish_conversion') as mock_finish:
            self.controller._on_process_finished(mock_process, 0, "")
        
        # Should finish conversion when no more files
        mock_finish.assert_called_once()
    ...
    def test_on_process_finished_not_converting(self):
        ...
        mock_process_next.assert_not_called()
...
class TestSourceFileDeletion:
    ...
    @patch('os.unlink')
    def test_delete_source_file_success(self, mock_unlink, mock_exists):
        """Test successful source file deletion"""
        mock_exists.return_value = True
        
        file_path = "/test/video.ts"
        
        self.controller._delete_source_file(file_path)
    ...
    @patch('os.path.exists')
    @patch('os.unlink')
    def test_delete_source_file_not_exists(self, mock_unlink, mock_exists):
    ...
    @patch('os.path.exists')
    @patch('os.unlink')
    def test_delete_source_file_permission_error(self, mock_unlink, mock_exists):
    ...
    def test_delete_source_disabled(self):
        ...
        with patch('os.unlink') as mock_unlink:
            self.controller._delete_source_file("/test/video.ts")
...
class TestConversionValidation:
    ...
    def test_extreme_crf_values(self):
        ...
        result = self.controller.start_conversion(
            file_paths=file_paths,
            codec_idx=0, hwdecode_idx=0, crf_value=100,  # Very high CRF
            parallel_enabled=False, max_parallel=1,
            delete_source=False, overwrite_mode=True
        )
...
class TestConversionControllerEdgeCases:
    ...
    def test_process_finish_race_condition(self):
        ...
        # Should not process next when conversion stopped
        mock_process_next.assert_not_called()
    ...
    def test_memory_cleanup_on_large_queue(self):
        ...
        result = self.controller.start_conversion(
            ...
            codec_idx=0, hwdecode_idx=0, crf_value=18,
            parallel_enabled=True, max_parallel=8,
            delete_source=False, overwrite_mode=True
        )
...
class TestConversionControllerIntegration:
    ...
    def test_full_conversion_workflow(self, mock_extension, mock_encoder, mock_audio, mock_hw):
        ...
        result = self.controller.start_conversion(
            file_paths=file_paths,
            codec_idx=0, hwdecode_idx=1, crf_value=18,
            parallel_enabled=True, max_parallel=2,
            delete_source=False, overwrite_mode=True
        )
        ...
        self.mock_process_manager.start_batch.assert_called_once_with(file_paths, True, 2)
        
        # Should have started processing
        assert len(self.controller.queue) <= len(file_paths)  # Queue consumed by _process_next
