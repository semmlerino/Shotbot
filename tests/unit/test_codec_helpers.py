...
class TestHardwareAcceleration:
    ...
    def test_hardware_acceleration_timeout(self, mock_subprocess):
        ...
        assert len(args) == 0  # Should fallback to no hardware acceleration
...
class TestEncoderConfiguration:
    ...
    def test_h264_nvenc_configuration(self, mock_subprocess):
        ...
        args, message = CodecHelpers.get_encoder_configuration(1, 4, False, 18)  # H.264 NVENC
    ...
    def test_hevc_nvenc_configuration(self, mock_subprocess):
        ...
        args, message = CodecHelpers.get_encoder_configuration(2, 4, False, 18)  # HEVC NVENC
    ...
    def test_av1_nvenc_configuration(self, mock_subprocess):
        ...
        args, message = CodecHelpers.get_encoder_configuration(3, 4, False, 18)  # AV1 NVENC
    ...
    def test_x264_software_configuration(self):
        ...
        args, message = CodecHelpers.get_encoder_configuration(0, 6, False, 18)  # x264
    ...
    def test_x264_parallel_no_threads(self):
        ...
        args, message = CodecHelpers.get_encoder_configuration(0, 6, True, 18)  # x264, parallel
        
        assert "-c:v" in args
        assert "libx264" in args
        assert "-threads" not in args  # Should not set threads in parallel mode
    ...
    def test_prores_configuration(self, mock_subprocess):
        ...
        args, message = CodecHelpers.get_encoder_configuration(4, 4, False, 18)  # ProRes
    ...
    def test_encoder_fallback_to_x264(self, mock_subprocess):
        ...
        args, message = CodecHelpers.get_encoder_configuration(1, 4, False, 18)  # H.264 NVENC (unavailable)
        
        assert "-c:v" in args
        assert "libx264" in args  # Should fallback
    ...
    def test_encoder_configuration_exception(self):
        ...
        with patch('subprocess.check_output', side_effect=subprocess.CalledProcessError(1, "ffmpeg")):
            ...
            assert "libx264" in args
            assert "-crf" in args
            assert "23" in args  # Safe default CRF
            assert "error" in message.lower()
...
class TestThreadOptimization:
    ...
    def test_single_cpu_job_auto_threads(self):
        """Test single CPU job uses auto-detection"""
        threads = CodecHelpers.optimize_threads_for_codec(3, False, None)  # x264, not parallel
        assert threads == 0  # Auto-detect
    ...
    def test_parallel_cpu_thread_division(self, mock_cpu_count):
        ...
        # 3 CPU jobs, 16 cores -> 16/3 = 5.33 -> max(2, 5) = 5
        assert threads == 5
    ...
    def test_cpu_count_none_fallback(self, mock_cpu_count):
        ...
        expected = max(2, ProcessConfig.OPTIMAL_CPU_THREADS // 1)
...
class TestRTX40Detection:
    ...
    @pytest.mark.parametrize("gpu_model,expected", [
        ("RTX 4090", True),
        ("RTX 4080", True),
        ("RTX 4070", True),
    ...
    def test_hardware_configurations(self, hardware_config):
        ...
        with patch('subprocess.check_output') as mock_subprocess:
            ...
            mock_subprocess.side_effect = [
                hardware_config['encoder_detection'](),
                hardware_config['gpu_detection']().encode() if callable(hardware_config['gpu_detection']) else hardware_config['gpu_detection']
            ]
...
class TestCodecHelpersEdgeCases:
    ...
    def test_crf_value_clamping(self):
        """Test CRF value clamping in encoder configuration"""
        with patch('subprocess.check_output', return_value=MockEncoderDetection.full_nvenc_support()):
            # Test high CRF value gets clamped
            args, _ = CodecHelpers.get_encoder_configuration(1, 4, False, 100)
            
            cq_idx = args.index("-cq") + 1
            cq_value = int(args[cq_idx])
            assert cq_value <= 51  # Should be clamped to max
