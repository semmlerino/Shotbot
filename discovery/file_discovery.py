...
def find_mov_file_for_path(thumbnail_path: Path) -> Path | None:
    ...
    try:
        ...
        if not version_dir:
            logger.debug(f"Could not find version directory for path: {thumbnail_path}")
            return None
        ...
        return _find_mov_in_dir(mov_dir)
...
def find_plate_mov_proxy(
    ...
    try:
        ...
        # Find MOV files in the directory
        mov_file = _find_mov_in_dir(mov_dir)
        if mov_file is None:
            return None
...
def find_plate_exr_sequence(
    ...
    try:
        ...
        from discovery.plate_finders import get_highest_resolution_dir
